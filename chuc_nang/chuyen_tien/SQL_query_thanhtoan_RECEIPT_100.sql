...
LEFT JOIN (SELECT MONEY_310.receipt_100_r_id,SUM(MONEY_310.reconcile_amount) AS total_reconcile_amount
           FROM  MONEY_310
		   WHERE MONEY_310.organization_000_d_id = "1b19126e-03da-42b1-bdee-4a1469016739"
...
WHERE RECEIPT_100.billing_date IS NOT NULL AND RECEIPT_100.association_return_flag = 0 AND RECEIPT_100.delete_flag = 0 
		       AND RECEIPT_100.organization_000_d_id = "1b19126e-03da-42b1-bdee-4a1469016739" AND RECEIPT_100.payment_date IS NULL 
			   AND RECEIPT_100.insurer_return_flag = 0 AND ((RECEIPT_100.billing_date >= "2019/01/01" 
			   AND RECEIPT_100.billing_date <= "2020/01/10")) AND ((RECEIPT_100.insurance_category_number >= 1 
			   AND RECEIPT_100.insurance_category_number <= 9))
			   AND ((INSURER_000.insurer_jurisdiction_tcd >= 1 
			   AND INSURER_000.insurer_jurisdiction_tcd <= 9)) AND (FACILITY_000.address_pref_tcd <> 2)
			   AND  (((INSURER_000.general_insurer_flag = 1) AND (INSURER_000.insurer_individual_number = 1))  
			   OR  ((INSURER_000.general_insurer_flag = 0) AND ((INSURER_000.insurer_individual_number >= 1 
			   AND INSURER_000.insurer_individual_number <= 9)))) AND (RECEIPT_100.association_pending_flag = 1)
			   AND ((PRACTITIONER_000.practitioner_number >= 1 AND PRACTITIONER_000.practitioner_number <= 9))
			   AND ((FACILITY_000.facility_number >= 1 AND FACILITY_000.facility_number <= 9))
			   AND ((RECEIPT_100.registered_symbol_number >= 1 
			   AND RECEIPT_100.registered_symbol_number <= 9)) 
			   AND ((FACILITY_010.operating_organization_code >= 1 
			   AND FACILITY_010.operating_organization_code <= 9))
